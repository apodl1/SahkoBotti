...
if __name__ == '__main__':
  ...
  elec_prices = ElecPrices() #prices fetched and sotred on construction
  application = ApplicationBuilder().token(telegram_token).build()

  #handlers for messages
  start_handler = CommandHandler('start', start)
  price_handler = CommandHandler('hinta', partial(price, prices_object=elec_prices)) #pass prices-object to callback via partial
  help_handler = CommandHandler('help', help)
  application.add_handler(start_handler)
  application.add_handler(price_handler)
  application.add_handler(help_handler)

  assert application.job_queue #this exists if requiremets installed
  # Schedule fetching new prices, the calLed function recurses and runs again in 1h if new prices not available (or fetch fails)
